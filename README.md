...
**TTN downlink — config over the air:** The device listens for downlinks on **FPort 3**. Two commands are supported: **(1) Set reporting interval:** 6-byte payload (hex): **`AA 55`** followed by 4-byte big-endian interval in **seconds**. Examples: 1 min = `AA550000003C`, 5 min = `AA550000012C`, 10 min = `AA5500000258`, 1 hour = `AA5500000E10`. **(2) Set seismic threshold (AT+SENS equivalent):** 4-byte payload (hex): **`AA 55 02 00`** (high) or **`AA 55 02 01`** (low). Example: `AA550201` for low sensitivity. In TTN Console go to your device → **Downlink** → FPort **3**, payload type **Hex**, enter the payload → Send. The device applies the change and persists it. Other settings (DR, TXP, etc.) are not changed by application downlinks.
